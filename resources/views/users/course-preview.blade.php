                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="list-group pb-2">
                                ...
                                <a href="{{ route('usercpreview', $course->id) }}" class="list-group-item list-group-item-action {{ Request::is('user/course-preview/'.$course->id) ? 'active' : '' }}">
                                  Overview
                                </a>
                                @foreach ($topics as $ta)
                                    @if($ta->status == 'active')
                                        <a href="{{ url('/') }}/user/course-preview/{{$course->id}}/{{$ta->id}}" class="list-group-item list-group-item-action {{ Request::is('user/course-preview/'.$course->id.'/'.$ta->id) ? 'active' : '' }}">{{ $ta->title }}</a>
                                    @endIf
                                @endforeach
                            </div>
                            {{-- End Menu Index --}}
